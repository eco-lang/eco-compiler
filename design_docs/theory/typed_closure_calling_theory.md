...
`eco.call` and `eco.papExtend` carry GC root operands supplied by `EcoGCPrepare`. Statepoint insertion itself is delegated to LLVM's `RewriteStatepointsForGC`: the non-leaf closure dispatch runtime functions (`eco_apply_closure`, `eco_apply_segmentation_unknown`, `eco_pap_extend`, `eco_closure_call_saturated`, and every `eco_alloc_*` boxing call) are declared **without** `gc-leaf-function`, so RS4GC automatically wraps each such call in a `gc.statepoint` that captures the live `ptr addrspace(1)` set across the call and emits `gc.relocate` for each on the return edge. The alloca'd args buffers inside those runtime helpers are also registered as shadow stack ranges (`eco_gc_push_stack_range`) around allocation, since static stack maps cannot describe them. `buildEvaluatorArgs` re-resolves the closure pointer from its HPointer before each `values[i]` load.
