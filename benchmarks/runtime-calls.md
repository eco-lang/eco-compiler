...
**Engine.** Selected by `ECO_MONO_ENGINE` (unset = `subst`, the shipping default =
"Run A"; `solver` = "Run B", which additionally exercises LSS stamps / the `fast`
counter). Always wipe `eco-stuff` when changing engine (the cache is engine-blind).
...
stamped direct `$cap` calls (LSS coverage, needs a binary built with
`ECO_LSS_DISPATCH_SITE_COUNTERS=1` — E0.4, not yet wired, so `fast=0` today).
Sanity invariant: `gen ≤ sat`.

**Commands** (run from `/work`):

```bash
# After any runtime (RuntimeExports.cpp) change, rebuild + FORCE-relink Stage 6
# (the CMake dep graph does not auto-relink eco-compiler against the runtime .a):
cmake --build build --target EcoRuntimeStatic
rm -f build/compiler/build-kernel/bin/eco-compiler
cmake --build build --target eco-compiler

# One cold Stage-7a run with the census on:
BK=build/compiler/build-kernel
rm -rf "$BK/eco-stuff"; rm -f "$BK/bin/eco-compiler-boot.mlir"   # cold module cache; keep ~/.eco
( cd "$BK" && ulimit -c 0 && ECO_DISPATCH_STATS=1 /usr/bin/time -v -o timing.txt \
    ./bin/eco-compiler make --optimize --kernel-package eco/compiler \
    --local-package eco/kernel=/work/eco-kernel-cpp \
    --output=bin/eco-compiler-boot.mlir /work/compiler/src/Terminal/Main.elm  2> census.log )

# Symbolize the top-N evaluators:
benchmarks/dispatch-census.sh "$BK/bin/eco-compiler" "$BK/census.log" 25
```
...
for that engine (subst = 12,007,395 B).
