...
module {
  func.func @main() -> i64 {
    ...
    %be_buf = bf.alloc %size : !eco.value
    %be_c0 = bf.cursor.init %be_buf : !eco.value -> !bf.cursor
    %be_c1 = bf.write.u16 %be_c0, %val (be) : !bf.cursor

    // Write as little-endian
    %le_buf = bf.alloc %size : !eco.value
    %le_c0 = bf.cursor.init %le_buf : !eco.value -> !bf.cursor
    %le_c1 = bf.write.u16 %le_c0, %val (le) : !bf.cursor

    // Read first byte of each
    %be_rc0 = bf.decoder.cursor.init %be_buf : !eco.value -> !bf.cursor
    %be_b1, %be_rc1 = bf.read.u8 %be_rc0 : i64, !bf.cursor

    %le_rc0 = bf.decoder.cursor.init %le_buf : !eco.value -> !bf.cursor
