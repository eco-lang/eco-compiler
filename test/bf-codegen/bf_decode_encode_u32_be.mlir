...
module {
  func.func @main() -> i64 {
    ...
    %new_buffer = bf.alloc %size : !eco.value
    %new_cursor0 = bf.cursor.init %new_buffer : !eco.value -> !bf.cursor
    %new_cursor1 = bf.write.u32 %new_cursor0, %val (be) : !bf.cursor

    // Verify by comparing individual bytes
    %orig_read0 = bf.decoder.cursor.init %buffer : !eco.value -> !bf.cursor
    %new_read0 = bf.decoder.cursor.init %new_buffer : !eco.value -> !bf.cursor
