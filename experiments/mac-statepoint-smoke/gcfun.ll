; E-M1 statepoint smoke — input IR, pre-RewriteStatepointsForGC.
;
; Two functions using the "statepoint-example" GC strategy. run.sh pipes this
...
; live GC root, then `llc` emits the __LLVM_STACKMAPS section describing
; where %obj lives at each safepoint. The harness (harness.cpp) implements
; do_safepoint() by unwinding the stack and recovering %obj from the stackmap.
;
; consume_root:           plain frame — root expected in a spill slot or
;                         callee-saved register relative to fp/sp.
; consume_root_dynalloca: dynamic + overaligned alloca live across the
;                         safepoint — on arm64 this forces a base pointer
;                         (x19), so stackmap locations may be x19-relative.
;                         This is the case the Eco StackMap consumer must
;                         handle per the build-on-mac plan.
