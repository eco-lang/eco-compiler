...
**IMPORTANT — Stage 2 is the true clean-build baseline.** The Stage 5
MLIR is generated by the JS compiler produced in Stages 2–4. If your fix
touches Elm compiler sources (anything under `compiler/src/`), the JS
compiler itself must incorporate those changes before it can generate
correct MLIR. That means re-running from **Stage 2** (not Stage 5):

  Stage 2 → rebuild JS compiler with the fix
  Stages 3–4 → fixed-point verify
  Stage 5 → generate MLIR using the fixed JS compiler
  Stage 6 → compile MLIR to native
  Stage 7 → test

Skipping Stage 2 and jumping straight to Stage 5 with a stale JS
compiler will produce MLIR from the *old* code, masking or misattributing
failures.
