...
mechanism); only the latent validate-walker zero-stride fix it uncovered is
retained (`NurserySpace.cpp` old-gen check walker: tag==0 skips 8 bytes —
a boxed-Int value word otherwise decodes as size-0 and hangs the stride).
