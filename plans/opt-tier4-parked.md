...
| **Oracle-coupled reification on self-compile** (any transform requiring borrow analysis at compile time — incl. tier-1's U-T1.4 call-crossing promotion) | the ~15% analysis wall toll vs 1–5% realistic wins (tier-1 §0 economics; T-DECISION-1 off-by-default); **then killed on the merits 2026-08-07: the OC1.0 reason census (`borrow-oracle-consumers.md`, Run K in `benchmarks/tier2-opt.md`) measured the borrowed-splittable+captured pool at 0.03% weighted vs the ≥3% gate on BOTH engines — the call-entangled mass is `owned` 15–18% (callees genuinely retain) + `closurecall` 9–11% (statically unknown callee) + `opaque` 5–6%** | a user workload whose OC1.0 census clears 3% — the ONLY remaining arm: OC1.0b ran 2026-08-07 (Run L) and closed the `closurecall` bucket too (100% `unres-dyn`: no singleton anno, no stamp at any blocking site — AbiCloning already devirtualized everything resolvable; `lam-splittable` = 0%) |
